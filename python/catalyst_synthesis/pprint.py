...
def pstr_expr(expr:ExprLike,
    ...
    if isinstance(e, FCallExpr):
        arg_expr, kwarg_expr = e.args, e.kwargs
        ...
        elif len(arg_expr)==2 and _isinfix(e):
            return acc_body, f"{_parens(arg_expr[0], args[0], opt)} {e.expr.vname.val} {_parens(arg_expr[1], args[1], opt)}"
        else:
            return acc_body, f"{e.expr.vname.val}({', '.join(args + [(k+'='+v) for k,v in kwargs])})"
        ...
        if _style(e.style, opt) == ControlFlowStyle.Python:
            st1 = st.tabulate()
            accArg = pstr_stmt(assignStmt(e.statevar, e.arg), st, opt) if e.statevar else []
            ...
            accArg, sarg = pstr_poi(e.arg, st, opt)
            accL, lexprL = pstr_poi(e.lbound, st, opt)
            accU, lexprU = pstr_poi(e.ubound, st, opt)
        ...
        # assert len(kwarg_expr)==0
        if _style(e.style, opt) == ControlFlowStyle.Python:
            accPoi, spoi = pstr_poi(e.arg, st, opt)
...
def pstr_poi(p:POI, state=None, opt=None) -> Tuple[List[str],str]:
    """ Pretty-print the point of insertion. """
    st = state if state is not None else PStrState(0,Suffix(0))
    (lines, e) = pstr_expr(p.expr, st, opt) if p.expr else ([], "None")
...
def pstr(p:Union[Builder, Program, Stmt, ExprLike, POI], default_cfstyle=DEFAULT_CFSTYLE) -> str:
    ...
    if isinstance(p, Builder):
        ...
        if isinstance(p, Program):
            ...
            stmts,tail = pstr_expr(p, None, opt)
            return '\n'.join(stmts + [f"## {tail} ##"])
        elif isinstance(p, POI):
            stmts,tail = pstr_poi(p, None, opt)
            return '\n'.join(stmts + [f"## {tail} ##"])
        else:
            assert_never(p)
